...
struct ShoppingListGeneratorTests {
    @Test @MainActor func generatesItemsFromPlannedMeals() throws {
        ...
        // 500 g flour (Grain) → purchase catalog: lb, 0.25 inc → 1.102 lb → snap 1.25 lb
    ...
    @Test @MainActor func subtractsInventoryQuantities() throws {
        ...
        let items = try context.fetch(FetchDescriptor<ShoppingListItem>())
        #expect(items.count == 1)
        // 300 g flour (Other) → purchase catalog: oz, inc 1 → 10.58 oz → snap 11 oz
        #expect(items[0].unit == "oz")
        #expect(abs(items[0].quantity - 11.0) < 0.01)
    }
    ...
    @Test @MainActor func servingMultiplierScalesQuantity() throws {
        ...
        let items = try context.fetch(FetchDescriptor<ShoppingListItem>())
        #expect(items.count == 1)
        // 200 g pasta (Other) → purchase catalog: oz, inc 1 → 7.05 oz → snap 8 oz
        #expect(items[0].unit == "oz")
    ...
    @Test @MainActor func inventoryDeductionMatchesUnit() throws {
        ...
        let items = try context.fetch(FetchDescriptor<ShoppingListItem>())
        #expect(items.count == 1)
        // 500 g flour (Other) → purchase catalog: oz, inc 1 → 17.64 oz → snap 18 oz
        #expect(items[0].unit == "oz")
        #expect(abs(items[0].quantity - 18.0) < 0.01)
    }
